function varargout=inferGlyProd_update(substrObj,enzObj,max_size)
% inferGlyProd infer the product based on the substrate and enzyme.

%Works!

%Update1:   Added max_size to input arguments, this will eventually allow me
%           to terminate a one compartment polymerization loop, although it
%           will probably have to be for the inferGlyForwPath.m
%           NOTE: now this function will not work for GHEnz
    ...
    if(enzFuncToAdd==1)
        for i = 1 :numRes
            ...
            if ~isempty(linkagechildren)
                for i=1:numBonds
                    linkagechildren(i).bonds.posParent;
                    carbon_bond_num=[carbon_bond_num,linkagechildren(i).bonds.posParent];
                end
            end
            
            bond_to_create=enzObj.linkFG.bond.posParent;
            
            %rule 2.1: cannot add to the same spot if a resiude is already
            %attached there.
            
            if ismember(bond_to_create,carbon_bond_num)
                isValidRxnPos = false;
            else
                isValidRxnPos = true;
            end
            ...
            %rule 2.2: cannot add if x number of residues are already
            %present (length(linkageChildren)
            ...
            if numRes-1>=2 %remember there is always an extra residue called "Free End" that is why -1
