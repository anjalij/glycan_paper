function varargout = inferGlyForwPath_wo_specificity_1(substrObjArray,enzObjArray,max_size)
%inferGlyForwPath infer the pathway based on enzymes and glycans as starting structures.
%

%This program works with inferGly_Prod_update3.m in order to prevent the
%addition of the same substrate infinite number of times and different
%substrates to the same carbon.

%still lacking a way to terminate actually infinite loops....
...
if(nargin==4)
    ...
    if(strcmp(optionformat,'composition'))
...
for i=1:length(enzObjArray)
    ...
    [numProds,prodSpecies,rxns,glypath]...
        = inferGlyProd_wo_specificity(singleGlycan,singleEnz,max_size);
