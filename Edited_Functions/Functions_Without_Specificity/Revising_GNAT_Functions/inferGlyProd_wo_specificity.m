function varargout=inferGlyProd_wo_specificity_1(substrObj,enzObj,max_size)

%Right now appears to be working

% UPDATE 1: this did does not yet work with inferGlyForwPath because
%of the maxsize input.

%Update 1.5: so I just hand-put in a max-size of 8 because those are the
%biggest single compartment enzymes.

%Update2:   Added a condition to check if something is already attached at a
%           site and reject target residue if that is the case.

%Update3:   Added a condition that doesn't allow more than two branches on
%           a vertex...can change this later if I want.


% Update4: I have commented out all of the extra stuff here....

% Update 5: Trying to insert the max_size as an argument
...
if(isSubstValid)
    if(enzFuncToAdd==1)
        for i = 1 :numRes
            ...
            %Rule 2.1: defined somethings for 2.1 Also, the easier for
           
            
            %i=1:numBonds was leading to unknown problems...Changed this to
            %the if lengh(linkagechildren)==1;
           
            
            %the else statement means that this rule says that you cannot
            %add to a monosaccharide that already has two branches;
            
            %if you want to increase this, easy fix=if
            %length(linkagechildren)==2
            %   carbon_bond_num=[carbon_bond_num,linkagechildren(1).bonds.posParent]
            %   carbon_bond_num=[carbon_bond_num,linkagechildren(2).bonds.posParent]
            
            %etc.
            
            
            linkagechildren=residue.linkageChildren;
            carbon_bond_num=[];
            numBonds=length(linkagechildren);
            isValidRxnPos=true;
                    
            if ~isempty(linkagechildren)
                if length(linkagechildren)==1;
                    carbon_bond_num=[carbon_bond_num,linkagechildren(1).bonds.posParent];
                else
                    isValidRxnPos = false;
                end
            ...
            %rule 2.2: cannot add to the same spot if a resiude is already
            %attached there.
            
             bond_to_create=enzObj.linkFG.bond.posParent; %bond that will be created by enzyme
             
            if ismember(bond_to_create,carbon_bond_num) %checking to see if that bond is already filled
                isValidRxnPos = false;
            else
                isValidRxnPos = true;
            end
...
%}            
            ...
            % rule 11:the enzyme should not act if the size is too big
            if numRes-1>=max_size %remember there is always an extra residue called "Free End" that is why -1
